              ...
              <div class="overflow-auto" id="building_register">
                <table class="table">
                  <thread>
                    ...
                    @foreach($data as $row)
                      <tr>
                        <td><a href="/premises/{{$row->premesisID}}">1</a></td>
                        <td><a href="/premises/{{$row->premesisID}}">{{ $row->orgName }}</a></td>                    
                        <td><a href="/premises/{{$row->premesisID}}">{{ $row->premisesAdr . ', ' . $row->addressL1 . ', ' . 
                        $row->addressL2 . ', ' . $row->town . ', ' . $row->county . ', ' . $row->postCode }}</a></td>
                        <td><a href="/premises/{{$row->premesisID}}">{{ $row->foreName . ' ' . $row->surName }}</td>
                        <td><a href="/premises/{{$row->premesisID}}">{{ $row->monitorDate }}</a></td>
                      </tr>
                    @endforeach
                  </thread>
                  <tbody>
